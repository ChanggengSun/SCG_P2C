...
from dataprocess.common import npcal_pose0to1, CATEGORY_TO_INDEX
from linefit import ground_seg

GROUNDSEG_config = str((Path(PARENT_DIR) / "configs" / "others" / "nuscenes.toml").resolve())
...
def process_log(
    ...
    scene_num_id: int,
    output_dir: Path,
    resample2frequency=10,
    flow_only: bool = False,
    compression: Optional[str] = None,
    compression_level: int = 4,
    deltaflow_format: bool = False,
    ...
    def create_group_data(
        ...
        group.create_dataset('lidar', data=pc.astype(np.float32), **dataset_kwargs)
        group.create_dataset('pose', data=pose.astype(np.float64), **dataset_kwargs)
        group.create_dataset('ground_mask', data=gm.astype(bool), **dataset_kwargs)

        if pc_next is not None:
            group.create_dataset('lidar_next', data=pc_next.astype(np.float32), **dataset_kwargs)
        if ego_motion is not None:
            group.create_dataset('ego_motion', data=ego_motion.astype(np.float32), **dataset_kwargs)

        if flow_0to1 is not None:
            # ground truth flow information
            group.create_dataset('flow', data=flow_0to1.astype(np.float32), **dataset_kwargs)
            group.create_dataset('flow_is_valid', data=flow_valid.astype(bool), **dataset_kwargs)
            group.create_dataset('flow_category_indices', data=flow_category.astype(np.uint8), **dataset_kwargs)
            group.create_dataset('flow_instance_id', data=flow_instance.astype(np.int16), **dataset_kwargs)

        if deltaflow_format:
            group.create_dataset('pc0', data=pc.astype(np.float32), **dataset_kwargs)
            group.create_dataset('pose0', data=pose.astype(np.float32), **dataset_kwargs)
            ...
            group.create_dataset('pc1', data=pc_next.astype(np.float32), **dataset_kwargs)
            group.create_dataset('pose1', data=pose_next.astype(np.float32), **dataset_kwargs)

            history_pcs = list(history_pcs) if history_pcs is not None else []
            history_poses = list(history_poses) if history_poses is not None else []
            for h_idx in range(1, history_frames + 1):
                if h_idx <= len(history_pcs):
                    h_pc = history_pcs[h_idx - 1]
                else:
                    h_pc = pc
                if h_idx <= len(history_poses):
                    h_pose = history_poses[h_idx - 1]
                else:
                    h_pose = pose

                group.create_dataset(f'pch{h_idx}', data=h_pc.astype(np.float32), **dataset_kwargs)
                group.create_dataset(f'poseh{h_idx}', data=h_pose.astype(np.float32), **dataset_kwargs)

            if scene_id is not None:
                group.attrs['scene_id'] = str(scene_id)
    ...
    if flow_only:
        expected_timestamps = []
        for sweep_idx in range(len(sweep_data_lst) - 1):
            ts = sweep_data_lst[sweep_idx]['timestamp']
            if if_annotated_frame(sample_ann_dict, ts):
    ...
    required_delta_keys = [
        'pc0', 'pc1', 'pose0', 'pose1',
        *[f'pch{i}' for i in range(1, history_frames + 1)],
        *[f'poseh{i}' for i in range(1, history_frames + 1)],
    ]

    if check_h5py_file_exists(output_dir/f'{log_id}.h5', expected_timestamps):
        if not deltaflow_format:
            print(f'{log_id} already exists and all timestamps are , skip...')
            return

        need_reprocess = False
        try:
            with h5py.File(output_dir/f'{log_id}.h5', 'r') as f_exist:
                for ts in expected_timestamps:
                    ts_str = str(ts)
                    if ts_str not in f_exist:
                        need_reprocess = True
                        break
                    group_exist = f_exist[ts_str]
                    for key in required_delta_keys:
                        if key not in group_exist:
                            need_reprocess = True
                            break
                    if need_reprocess:
                        break
        ...
        if need_reprocess:
            os.remove(output_dir/f'{log_id}.h5')
        else:
            print(f'{log_id} already exists with DeltaFlow format keys, skip...')
            return

    pc_cache = {}
    pose_cache = {}

    def _get_cached_pc(sweep_data_item):
        ts = sweep_data_item['timestamp']
        if ts not in pc_cache:
            pc_val = _load_points_from_file(os.path.join(nusc.dataroot, sweep_data_item['filename']))
            pc_val, _ = remove_ego_points(pc_val)
            pc_cache[ts] = pc_val
        return pc_cache[ts]

    def _get_cached_pose(sweep_data_item):
        ts = sweep_data_item['timestamp']
        if ts not in pose_cache:
            pose_cache[ts] = get_pose(nusc, sweep_data_item)
        return pose_cache[ts]

    def _get_history(cnt_idx, current_pc, current_pose):
        if history_frames <= 0:
            return [], []
        ...
        history_poses = []
        for h_idx in range(1, history_frames + 1):
            hist_idx = cnt_idx - h_idx
            if hist_idx < 0:
                history_pcs.append(current_pc.copy())
                history_poses.append(current_pose.copy())
                continue

            hist_sweep = sweep_data_lst[hist_idx]
            hist_pc = _get_cached_pc(hist_sweep)
            if hist_pc.shape[0] < 1:
                hist_pc = current_pc
            history_pcs.append(hist_pc)
            history_poses.append(_get_cached_pose(hist_sweep))

        return history_pcs, history_poses

    with h5py.File(output_dir/f'{log_id}.h5', 'a') as f:
        for cnt, sweep_data in enumerate(sweep_data_lst):
            ts0 = sweep_data['timestamp']

            # lidar point cloud and pose of current frame
            pc0 = _get_cached_pc(sweep_data)
            if pc0.shape[0] < 10:
                print(f'{log_id}/{ts0} has no points....')
                continue
            pose0 = _get_cached_pose(sweep_data)
            is_ground_0 = mygroundseg.run(pc0[:, :3])

            history_pcs, history_poses = [], []
            if deltaflow_format:
                history_pcs, history_poses = _get_history(cnt, pc0, pose0)
            ...
            if cnt == len(sweep_data_lst) - 1:
                ...
                create_group_data(
                    group=group,
                    pc=pc0,
                    gm=is_ground_0.astype(np.bool_),
                    pose=pose0.astype(np.float32),
                    pc_next=pc0 if deltaflow_format else None,
                    pose_next=pose0.astype(np.float32) if deltaflow_format else None,
                    history_pcs=history_pcs if deltaflow_format else None,
                    history_poses=history_poses if deltaflow_format else None,
                    scene_id=log_id if deltaflow_format else None,
                )
                continue

            sweep_data_next = sweep_data_lst[cnt + 1]
            ts1 = sweep_data_next['timestamp']
            pose1 = _get_cached_pose(sweep_data_next)
            gt_flow_flag = if_annotated_frame(sample_ann_dict, ts0)

            if flow_only and not gt_flow_flag:
                continue

            group = f.create_group(str(ts0))

            pc1 = None
            if gt_flow_flag or deltaflow_format:
                pc1 = _get_cached_pc(sweep_data_next)
            ...
            if not gt_flow_flag:  # no annotations, only save data
                create_group_data(
                    ...
                    pc_next=pc1,
                    pose_next=pose1.astype(np.float32) if deltaflow_format else None,
                    history_pcs=history_pcs if deltaflow_format else None,
                    history_poses=history_poses if deltaflow_format else None,
                    scene_id=log_id if deltaflow_format else None,
                )
                ...
                create_group_data(
                    ...
                    gm=is_ground_0.astype(np.bool_),
                    pose=pose0.astype(np.float32),
                    flow_0to1=scene_flow['flow_0_1'],
                    flow_valid=scene_flow['valid_0'],
                    flow_category=scene_flow['classes_0'],
...
def process_logs(
    ...
    args = sorted([
        ...
        for scene_num_id in range(len(scene_list))
    ])
...
def main(
    ...
    compression: Optional[str] = None,
    compression_level: int = 4,
    write_index_total: bool = True,
    deltaflow_format: bool = False,
    history_frames: int = 3,
):
    if isinstance(compression, str) and compression.strip().lower() in ("", "none", "null"):
        compression = None
    history_frames = max(0, int(history_frames))
    ...
    for input_key, input_val in input_dict.items():
        ...
        process_logs(
            ...
            deltaflow_format=deltaflow_format,
